# ==============================================================================
# Step 4.2 - Aquatic ecosystem pressure index by micro-watershed (VERSION 2)
#
# Same inputs, sign orientation and NA handling as 4.1_pressure_index.R. The
# ONLY difference is the aggregation:
#
#   4.1 (v1) -> mean WITHIN each theme first, then across the 5 themes, so
#               every theme weighs 20% regardless of how many metrics it
#               contributes.
#   4.2 (v2) -> plain mean of the 15 sub-indicators, each weighing 1/15.
#
# Implied theme weights in this version:
#   uso del suelo   5/15 = 33.3%      deficit hidrico  2/15 = 13.3%
#   fragmentacion   4/15 = 26.7%      poblacion        1/15 =  6.7%
#   caudal          3/15 = 20.0%
#
# The practical effect versus v1 is that population stops dominating: in v1
# it carried a full 20% on its own, and because GHS-POP is a smooth modelled
# product its tau saturates at exactly 1.0 in most inhabited basins, which
# made the index behave largely like a "does this basin have people?" map.
# Here that same signal carries 6.7%.
#
# Sign orientation
# ------------------------------------------------------------------------------
...
# "pd" metric is the main offender, ~213 basins). Those are numerical
# artefacts rather than trends, so they are demoted to NA before anything
# else and then treated like any other constant series.
#
# Since every ori_i is in [-1, 1], their mean is too: the index is bounded by
# [-1, 1], with +1 = maximum pressure.
#
# Output (workspace/11_pressure_index_v2_by_basin.gpkg), one feature per
...
#                       preserved here (constant/short series, an invalid
#                       tau, or a basin absent from that source table).
#   ori_<indicator>   - Same value re-oriented towards pressure
#                       (direction * tau) with NA replaced by 0. These 15
#                       columns are what the index averages.
#   tema_*            - Mean of the ori_* values of each theme. DIAGNOSTIC
#                       ONLY in this version: unlike 4.1, they do not feed
#                       indice_presion. Kept so the themes can still be
#                       mapped and compared.
#   indice_presion    - Final index: plain mean of the 15 ori_* columns.
...
output_path <- "workspace/11_pressure_index_v2_by_basin.gpkg"
...
# extraction, orientation and aggregation below are all driven by it. In this
# version every row weighs the same (1/15), so adding a row to a theme also
# increases that theme's total influence.
...
# ------------------------------------------------------------------------------
# 6. Aggregate: plain mean of the 15 sub-indicators (equal weight, 1/15 each)
# ------------------------------------------------------------------------------
...
# THE INDEX: every sub-indicator counts exactly the same. This is the single
# line that differs from 4.1_pressure_index.R.
wide$indice_presion <- rowMeans(as.matrix(wide[, ori_cols, drop = FALSE]))

# Theme scores are still computed, but only as a diagnostic aid for mapping
# and interpretation - they do NOT feed indice_presion in this version.
for (tm in temas) {
  tm_cols <- paste0("ori_",
                    subindicators$indicador[subindicators$tema == tm])
  wide[[paste0("tema_", tm)]] <-
    rowMeans(as.matrix(wide[, tm_cols, drop = FALSE]))
}
