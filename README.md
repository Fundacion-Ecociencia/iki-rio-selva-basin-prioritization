...
- **`1.1_lulc_composition.R` writes a filename its own output constant doesn't match downstream
  usage.** It declares `output_path <- "workspace/01_lulc_by_microbasin.csv"`, but every downstream
  script reads `workspace/01_lulc_by_basin.csv`. Only the latter is currently tracked; re-running
  `1.1` as-is will not update the file the rest of the pipeline actually consumes.
- **A handful of `mk_tau` values in `05_fragmentation_trends_by_basin.csv` are numerically
  invalid** (non-finite or outside [-1, 1]), from `trend::mk.test` on heavily tied series —
  primarily the `pd` (patch density) metric. Both pressure-index scripts detect and null these out
  before aggregating; any other consumer of that table should do the same.
- **Basin coverage differs by source table.** The discharge and deficit tables (`06`, `07`) cover
  the full GEOGloWS domain (~12,400 `comid`), while the land-cover, fragmentation and population
  tables cover the ~1,747-basin study area (fragmentation is short by 5 basins). Every join in this
  pipeline is anchored on the basins layer, so the study area — not the largest table — always
  defines the universe.
- **The population trend signal is close to binary.** GHS-POP is a smooth, modelled product, so its
  Mann-Kendall tau saturates at exactly 1.0 in most inhabited basins and is `NA` where population is
  zero throughout. Under the 20%-per-theme weighting of index v1, this made population dominate the
  ranking; index v2's equal per-sub-indicator weighting was added specifically to address that.
